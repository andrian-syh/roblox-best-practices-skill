...
5. Set `Visible = false` on closed panels and destroy screens that will not reopen; an invisible frame still costs layout if it stays laid out.
...
6. Reserve `Players:BanAsync` for high-confidence signals; its alt-account and device options are strong medicine.
...
6. For memory attribution, tag systems with `debug.setmemorycategory` so "LuaHeap is growing" becomes "the pet system is growing".
